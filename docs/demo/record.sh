...
ROOT="$(cd "$HERE/../.." && pwd)"
cd "$ROOT"
export DEMO_SHOW='uv run python hue_server.py   # depot hue-mcp, HUE_BRIDGE_IP/HUE_USERNAME dans l'environnement'
asciinema rec --overwrite --cols 100 --rows 30 --idle-time-limit 2 \
  --command "uv run python $HERE/mcp_demo.py 'uv run python hue_server.py' get_all_lights" /tmp/demo.cast
